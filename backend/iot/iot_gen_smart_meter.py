...
con_iot_device_id = "f531b9c1-c46a-42c4-989d-1d5be315f6a6" #Smart meters
        ...
        if auto_mode == 1:
            ...
            if iot_device_id == con_iot_device_id:
                ...
                voltage = {
                    ...
                    "sensor_type": "voltage",
                    "sensor_value": round(random.uniform(90, 130), 2), #VAC
                ...
                current = {
                    ...
                    "measured_at": str(datetime.now()),
                    "sensor_type": "current",
                    "sensor_value": round(random.uniform(0, 99), 2), #mA
                    "created_at": str(datetime.now()),
                    "auto_mode": 1,
                    "synced_flag": 0                    
                }
                ...
                lis_data.append(voltage)
                lis_data.append(current)
            ...
            if iot_device_id == con_iot_device_id:
                ...
                voltage_json = data.get("sensors", {}).get("voltage", None)  # Default to None if the key doesn't exist
                voltage = {
                    "data_id": str(uuid.uuid4()),
                    "device_id": iot_device_id,
                    "measured_at": str(datetime.now()),
                    "sensor_type": "voltage",
                    "sensor_value": round(voltage_json, 2),
                    "created_at": str(datetime.now()),
                    "auto_mode": 0,
                    "synced_flag": 0 
                }

                current_json = data.get("sensors", {}).get("current", None)  # Default to None if the key doesn't exist
                current = {
                    "data_id": str(uuid.uuid4()),
                    "device_id": iot_device_id,
                    "measured_at": str(datetime.now()),
                    "sensor_type": "current",
                    "sensor_value": round(current_json, 2),
                    "created_at": str(datetime.now()),
                    "auto_mode": 0,
                    "synced_flag": 0 
                }
